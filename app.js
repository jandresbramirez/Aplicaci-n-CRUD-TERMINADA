...
// Función para agregar un usuario
function addUser(req, res) {
    const { Cedula, Nombres, Apellidos, FechaDeNacimiento, Profesion, Expectativas, Correo, Contraseña } = req.body;

    // Validación de campos requeridos
    if (!Cedula || !Nombres || !Apellidos || !FechaDeNacimiento || !Profesion || !Expectativas || !Correo || !Contraseña) {
        return res.status(400).send('Todos los campos son obligatorios.');
    }

    const query = `INSERT INTO usuario (Cedula, Nombres, Apellidos, FechaDeNacimiento, Profesion, Expectativas, Correo, Contraseña) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;
    db.query(query, [Cedula, Nombres, Apellidos, FechaDeNacimiento, Profesion, Expectativas, Correo, Contraseña], (err) => {
        if (err) {
            console.error("Error al agregar usuario:", err);
            return res.status(500).send('Error al agregar usuario.');
        }
...
function updateUser(req, res) {
    ...
    const query = `UPDATE usuario SET Nombres = ?, Apellidos = ?, FechaDeNacimiento = ?, Profesion = ?, Expectativas = ?, Correo = ?, Contraseña = ? WHERE Cedula = ?`;
    db.query(query, [Nombres, Apellidos, FechaDeNacimiento, Profesion, Expectativas, Correo, Contraseña, Cedula], (err) => {
        if (err) {
            console.error("Error al actualizar usuario:", err);
            return res.status(500).send('Error al actualizar usuario.');
        }
